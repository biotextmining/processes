...
public class AutomatonMatcher extends Matcher {
	...
	public static AutomatonMatcher loadMatcher(CustomRunAutomaton[] automatons,boolean ignorecase){
		return new AutomatonMatcher(automatons, ignorecase);
	}
	...
	public List<Mention> match(String text, Document doc) {
		...
		for (int i = 0; i < automatons.length; i++){
			...
			com.silicolife.textmining.processes.ie.ner.linnaeus.adapt.dk.brics.automaton.CustomAutomatonMatcher matcher = automatons[i].newCustomMatcher(matchText);
