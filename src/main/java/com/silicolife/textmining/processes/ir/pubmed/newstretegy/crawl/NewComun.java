  	...
  	public static void borrarFicheroRecursivo(File file) {
  		if (file.isDirectory()) {
  			for (File subfile:file.listFiles()) {
  				NewComun.borrarFicheroRecursivo(subfile);
  			}
  		}
  		file.delete();
  	}
...
//	  {
		URL uri=new URL(url);
		if(!NewComun.urlEn(uri,urls)) urls.add(uri);
...
//	  }
  ...
  {
	return NewComun.indexOfIgnoreCase(cad,"<html")!=-1; //los html's contienen "<html"
