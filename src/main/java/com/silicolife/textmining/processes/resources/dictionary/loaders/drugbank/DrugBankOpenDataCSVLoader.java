...
public class DrugBankOpenDataCSVLoader extends DictionaryLoaderHelp implements IDicionaryFlatFilesLoader{
	...
	public IResourceUpdateReport loadTerms(IDictionaryLoaderConfiguration configuration) throws ANoteException, IOException {
		...
		for(String line:fileLines)
		{
			...
			if(step%100==0)
			{
				memoryAndProgress(starttime,  step, total);
			}
		}
		if (!cancel) {
	...
	private void processDrugLine(String line, boolean importExternlIDs) throws ANoteException {
		...
		String cas = lineSplitered[csvConfigurations.getColumsDelemiterDefaultValue().getColumnNameColumnParameters().get(columnCommonCASName).getColumnNumber()];
		...
		List<String> synonyms =  getSynonyms(line, lineSplitered);
	...
	private List<String> getSynonyms(String line,String[] lineSplitered)
	{
	...
	private boolean testFileHeader(File file)
	{
		try {
